...
import pydicom_v3_0_1  # for dcmwrite
from pydicom_v3_0_1 import jsonrep, config
from pydicom_v3_0_1._version import __version_info__
from pydicom_v3_0_1.charset import default_encoding, convert_encodings
from pydicom_v3_0_1.config import logger
from pydicom_v3_0_1.datadict import (
...
from pydicom_v3_0_1.dataelem import DataElement, convert_raw_data_element, RawDataElement
from pydicom_v3_0_1.filebase import ReadableBuffer, WriteableBuffer
from pydicom_v3_0_1.fileutil import path_from_pathlike, PathType
from pydicom_v3_0_1.misc import warn_and_log
from pydicom_v3_0_1.pixels import compress, convert_color_space, decompress, pixel_array
from pydicom_v3_0_1.pixels.utils import (
    reshape_pixel_array,
    get_image_pixel_ids,
    set_pixel_data,
)
from pydicom_v3_0_1.tag import Tag, BaseTag, tag_in_exception, TagType, TAG_PIXREP
from pydicom_v3_0_1.uid import PYDICOM_IMPLEMENTATION_UID, UID
from pydicom_v3_0_1.valuerep import VR as VR_, AMBIGUOUS_VR
from pydicom_v3_0_1.waveforms import numpy_handler as wave_handler
...
class Dataset:
    ...
    def __getitem__(self, key: "slice | TagType") -> "Dataset | DataElement":
        ...
        if isinstance(elem, RawDataElement):
            ...
            if elem.value is None and elem.length != 0:
                from pydicom_v3_0_1.filereader import read_deferred_data_element
            ...
            if self[tag].VR in AMBIGUOUS_VR:
                from pydicom_v3_0_1.filewriter import correct_ambiguous_vr_element
    ...
    def setdefault(self, key: TagType, default: Any | None = None) -> DataElement:
        ...
        ValueError
            ...
            keyword.
        KeyError
            If :attr:`~pydicom_v3_0_1.config.settings.reading_validation_mode` is
    ...
    def _convert_pixel_data_using_handler(self, name: str) -> None:
        ...
        if not handler.supports_transfer_syntax(tsyntax):
            raise NotImplementedError(
                ...
                f"handler '{name}'. Please see the pydicom_v3_0_1 documentation for "
                "information on supported transfer syntaxes."
            )
        if not handler.is_available():
            raise RuntimeError(
                f"The pixel data handler '{name}' is not available on your "
                "system. Please refer to the pydicom_v3_0_1 documentation for "
    ...
    def _convert_pixel_data_without_handler(self) -> None:
        ...
        possible_handlers = [
            hh
            for hh in pydicom_v3_0_1.config.pixel_data_handlers
        ...
        if not possible_handlers:
            raise NotImplementedError(
                ...
                "handlers available that support it. Please see the pydicom_v3_0_1 "
        ...
        logger.info(
            ...
            "pydicom-v3-0-1 documentation for alternative packages that might "
    ...
    def overlay_array(self, group: int) -> "numpy.ndarray":
        ...
        from pydicom_v3_0_1.overlays import get_overlay_array
    ...
    def _pretty_str(self, indent: int = 0, top_level_only: bool = False) -> str:
        ...
        ..versionchanged:: 2.0
            ...
            if :data:`~pydicom_v3_0_1.config.show_file_meta` is ``True`` (default)
        ...
        if (
            ...
            and self.file_meta
            and pydicom_v3_0_1.config.show_file_meta
        ):
    ...
    def __str__(self) -> str:
        ...
        ..versionchanged:: 2.0
            ...
            if :data:`pydicom_v3_0_1.config.show_file_meta` is ``True``
